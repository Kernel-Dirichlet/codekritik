...
def main():
    ...
    if args.log:
        log_base = 'logs_{}'.format(file_hash[:10])
        if not os.path.exists(log_base):
            os.makedirs(log_base)

        for subdir in ['abc_metrics', 'halstead_metrics',
                       'cyclomatic_complexity_metrics', 'loc_metrics']:
            subpath = os.path.join(log_base, subdir)
            if not os.path.exists(subpath):
                os.makedirs(subpath)

    # ── LOC ──────────────────────────────────────────────────────────────────
    full_loc_dict = None
    if runner_cfg.get('LOC', False):
        ...
        final_loc_dict = append_timestamp_hash(
            full_dict=full_loc_dict,
            timestamp=current_time,
            hash_=file_hash,
        )
        _ = dump_final_jsons(
            prefix_path='./{}/loc_metrics'.format(log_base),
            final_dicts=final_loc_dict,
        )

    # ── ABC ──────────────────────────────────────────────────────────────────
    if runner_cfg.get('ABC', False):
        abc_dict = abc_process_directory(
            directory=args.dir,
            extensions_to_count=extensions_to_count,
            extensions_map=extensions_map,
            hll_tokens='metrics_cfgs/hll_tokens.json',
        ...
        final_abc_dict = append_timestamp_hash(
            full_dict=full_abc_dict,
            timestamp=current_time,
            hash_=file_hash,
        )
        _ = dump_final_jsons(
            prefix_path='./{}/abc_metrics'.format(log_base),
            final_dicts=final_abc_dict,
        )

    # ── Halstead ─────────────────────────────────────────────────────────────
    full_halstead_dict = None
    if runner_cfg.get('Halstead', False):
        halstead_metrics = halstead_process_directory(
            args.dir,
            extensions_to_count,
            extensions_map,
            hll_tokens='metrics_cfgs/hll_tokens.json',
        ...
        full_halstead_dict = halstead_full_analysis(halstead_metrics, extensions_map)
        final_halstead_dict = append_timestamp_hash(
            full_dict=full_halstead_dict,
            timestamp=current_time,
            hash_=file_hash,
        )
        _ = dump_final_jsons(
            prefix_path='./{}/halstead_metrics'.format(log_base),
            final_dicts=final_halstead_dict,
        )

    # ── Cyclomatic Complexity ─────────────────────────────────────────────────
    full_cc_dict = None
    if runner_cfg.get('cyclomatic_complexity', False):
        cc_metrics = cc_process_directory(
            args.dir,
            extensions_to_count,
            extensions_map,
            hll_tokens='metrics_cfgs/hll_tokens.json',
        ...
        final_cc_dict = append_timestamp_hash(
            full_dict=full_cc_dict,
            timestamp=current_time,
            hash_=file_hash,
        )
        _ = dump_final_jsons(
            prefix_path='./{}/cyclomatic_complexity_metrics'.format(log_base),
            final_dicts=final_cc_dict,
        )

    # ── Maintainability Index ─────────────────────────────────────────────────
    if runner_cfg.get('Maintainability_index', False):
        missing = []
        if full_halstead_dict is None:
            missing.append('Halstead')
        if full_cc_dict is None:
            missing.append('cyclomatic_complexity')
        if full_loc_dict is None:
            missing.append('LOC')

        if missing:
            print(
                'WARNING: Maintainability_index requires {} to be enabled in runner_cfg. '
                'Skipping MI computation.'.format(', '.join(missing))
            )
        else:
            mi_metrics = full_maintainability_calc(
                full_halstead_dict=full_halstead_dict,
                full_cc_dict=full_cc_dict,
                full_loc_dict=full_loc_dict,
            )
            final_mi_metrics = append_timestamp_hash(
                full_dict=mi_metrics,
                timestamp=current_time,
                hash_=file_hash,
            )
            json.dump(
                final_mi_metrics,
                open('./{}/maintainability_metrics.json'.format(log_base), 'w'),
                indent=4,
            )
